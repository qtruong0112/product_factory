...
## MÀN KẾ TIẾP
**Nhóm "thư viện nền tảng" đã XONG, builder Pattern đã WIRE về DB thật, toàn bộ nhóm Pipeline sản phẩm (Template→Config→Variant→Catalog, Giai đoạn 14-17) ĐÃ HOÀN TẤT, Release + Activity Log (Giai đoạn 18) ĐÃ HOÀN TẤT.** Tiếp theo: **Simulation Engine** (`POST /api/simulation/run`, backend annuity dựa trên `simulation_scenario`/`simulation_schedule_row` đã có DDL+seed thật), rồi đợt polish cuối. Luôn kiểm tra bundler JS xem view liên quan có dùng state tĩnh không trước khi quyết định list-only hay list+detail; cũng kiểm xem cột nào trông như fabricate thực ra suy ra được thật (bài học "Kênh" ở Variant — suy ra từ `catalog_listing`; bài học "Kênh" ở Activity Log — suy ra bằng regex từ hậu tố "· kênh X" trong `detail`).
