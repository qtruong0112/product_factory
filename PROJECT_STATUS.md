...
### Giai đoạn 78 — Kích hoạt nút Thêm/Bớt Block (Pattern) + Thêm Config Fragment (Config), thuần UI không API/DB

**Bối cảnh:** user muốn ở Pattern builder có chức năng thêm/bớt Block, ở Config builder thêm được Config Fragment — nhấn mạnh rõ chỉ cần làm ở UI (state cục bộ), không tạo API mới/không ghi DB để tránh lỗi, và tuyệt đối không tạo UI mới vì các nút này đã có sẵn trên giao diện (dạng no-op theo đúng luật "Nút CUD: giữ giao diện, no-op, tooltip read-only").

**Sửa — Pattern (`ProductPatternDetailPage.tsx`):** thêm 2 handler `handleAddBlock`/`handleRemoveBlock`. `handleAddBlock` gọi `GET /api/blocks/{id}/detail` (API đọc đã có sẵn từ Giai đoạn 6/35, không tạo mới) lấy đúng Answer Slot thật của Block, rồi `setData` nối block mới vào `data.blocks` cục bộ. `handleRemoveBlock` lọc bỏ block khỏi `data.blocks`. Nối cả 3 điểm bấm có sẵn: palette Block (dấu +/✓, click để add/remove), nút thùng rác trên mỗi Block ở canvas, nút "+ Thêm Block" ở bảng độ phủ. `canvas`/`canvasIds`/`coverage` đều `useMemo` derive từ `data` nên tự phản ánh đúng ngay, không cần sửa logic độ phủ.

**Sửa — Config (`ProductConfigDetailPage.tsx`):** khối "Thêm Config Fragment" (trước đây `disabled` toàn bộ + mờ) bật hoàn toàn — chọn Selector Scope (Mặc định/Time/Place/People, đúng priority thật 0/1/2/3 theo bảng `selector_scope`), nhập Giá trị Scope + Giá trị gán, bấm "Thêm" gọi `handleAddFragment` đẩy 1 `FragmentRow` mới vào `data.slots[code].fragments` cục bộ, đồng bộ luôn `fragmentCount`/`sidebar` (dot "đã điền")/`missingRequired`/`completeness.pct` nếu là slot bắt buộc lần đầu có giá trị.

Cả 2 nơi **không gọi API ghi, không đổi DB** — state chỉ sống trong phiên trình duyệt, F5 sẽ mất (đúng chủ đích). Thêm dòng chú thích nhỏ "Minh hoạ trên UI — chưa ghi vào hệ thống" (Config) và đổi tooltip nút Block thành "Thêm/bớt tại UI — minh hoạ builder, chưa ghi vào hệ thống" (Pattern) thay cho tooltip "read-only" cũ, tránh gây hiểu nhầm đã lưu.

**Verify:** `npm run build` 0 lỗi TS; `docker compose up -d --build frontend`; smoke test 200 OK. Thuần frontend, không đổi backend/schema.

### Giai đoạn 79 — Khôi phục màu sắc + tăng kích thước ô cho Sysmap/Ontology

**Bối cảnh:** user gửi 5 ảnh chụp so sánh trực tiếp prototype gốc với hệ thống đang chạy, chỉ ra 2 màn "Sơ đồ quan hệ tổng thể" (Sysmap) và "Sơ đồ Ontology" thiếu màu sắc nghiêm trọng so với gốc.

**Sửa:** giải mã prototype gốc (`docs/Product Factory 5.1.html`, script Node `JSON.parse` trên nội dung `<script type="__bundler/template">`), đối chiếu `sysMapModel()`/`ontologyModel()`/`ONTO()` — phát hiện màu ở `SysmapPage.tsx` bị **đảo lộn sai hoàn toàn** so với gốc (archetype/obligation library/attribute dùng nhầm màu nhau), và card khái niệm ở `OntologyPage.tsx` mất hẳn cấu trúc 2 tầng màu (header nền màu + mô tả + pill đếm). Sửa lại đúng bảng màu gốc cho cả 2 file; thêm hàm `familyColorFor()` derive màu family (Term Loan/Revolving/Conditional) từ `archetypeName` thật có sẵn (không bịa field DB mới) áp cho bộ chọn Obligation Type + badge "THUỘC HỌ" mới. Khối "Cấu trúc..." (decomposition) **không sửa** — xác nhận qua API `obligation-element-types` cả 6 OET hiện tại đều `isIdentify=false` (đúng vì `OET_NATURE` đã gỡ từ Giai đoạn 52), thiếu màu ở đó là đúng dữ liệu thật. Sau đó user yêu cầu thêm "các ô các tab cho to hơn" — tăng padding/font-size/min-width 2 khối đầu Sysmap, khôi phục dòng mô tả phụ dưới mỗi ô pipeline đúng cấu trúc 2 dòng của gốc.

**Verify:** `npm run build` 0 lỗi TS; `docker compose up -d --build frontend`; smoke test 200 OK. Thuần frontend.

### Giai đoạn 80 — Đổi thương hiệu "Product Factory" → "FOMS"

**Bối cảnh:** user yêu cầu đổi tên logo/tab title/breadcrumb "home" sang "FINANCIAL OBLIGATION MANUFACTURING SYSTEM" viết hoa toàn bộ, sau đó tinh chỉnh: bỏ dòng phụ "LENDING OS", tab title rút gọn còn "FOMS", logo icon đổi từ "PF" sang "FOMS" màu xanh lá.

**Sửa:** `index.html` (`<title>`), `Layout.tsx` (logo sidebar, breadcrumb "home"), `favicon.svg` (viết lại hoàn toàn từ ký hiệu "F" cũ sang ô vuông gradient xanh + chữ "FOMS" trắng). Rà soát toàn bộ frontend+backend theo yêu cầu user, phát hiện sót 2 chỗ trong `SimulationPage.tsx` (nhãn xuất CSV/PDF) — đã hỏi qua AskUserQuestion, user chọn đổi luôn 2 chỗ đó, giữ nguyên README.md/package.json (codename nội bộ).

**Verify:** `npm run build` 0 lỗi TS mỗi lượt; `docker compose up -d --build frontend`; smoke test 200 OK. Thuần frontend.

### Giai đoạn 81 — Sửa lỗi NPE Simulation Engine khi chọn sản phẩm "hạn mức" không có Block Phí

**Bối cảnh:** user báo chọn vài sản phẩm để mô phỏng (cụ thể "Vay ô tô hạn mức") bị văng. Tái hiện bằng curl `GET /api/simulation/default?variantCode=VAR-102` → 500.

**Sửa:** `GlobalExceptionHandler` trước giờ nuốt mất stack trace — thêm `log.error("Unexpected error", ex)` (giữ lại lâu dài) rồi rebuild mới thấy root cause: `SimulationService.resolveSlotValue()` dùng `.stream().map(AnswerSlot::getDefaultValue).findFirst()` — khi `getDefaultValue()` trả `null`, `Stream.findFirst()` nội bộ gọi `Optional.of(null)` và ném NPE. Xác nhận qua `docker exec pf-db psql`: Answer Slot `fee_amount` (Block `BLK_FEE`) có `default_value = NULL` thật; VAR-102 dùng `CFG-0041` không có Block Phí trong cấu trúc nên rơi đúng nhánh lỗi. Sửa: thêm `s.getDefaultValue() != null` vào filter trước khi map — đối chiếu schema xác nhận đây là cột NULLABLE DUY NHẤT trong 3 tầng resolve (`fragment.value`/`template_frame.frame_value` đều NOT NULL).
