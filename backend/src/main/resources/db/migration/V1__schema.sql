...
-- Giai đoạn 42: role người dùng cho bộ chọn user demo ở sidebar (KHÔNG phải hệ thống đăng
-- nhập/bảo mật thật — chỉ lọc menu phía frontend). Tách riêng khỏi release_role_enum (đang
-- dùng cho process_step.role) để không ảnh hưởng dữ liệu Quy trình phát hành đã có.
...
-- Giai đoạn 42: người dùng thật cho bộ chọn "đổi vai trò" ở sidebar + nguồn populate
-- created_user/updated_user của các bảng nghiệp vụ chính (tên khớp đúng activity_log/version_entry
-- đã có sẵn từ trước, không phải nhân vật mới bịa ra — trừ 1 user Admin thêm mới).
CREATE TABLE "app_user" (
  "id" BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY NOT NULL,
  "code" varchar(20) UNIQUE NOT NULL,
  "name" varchar(120) NOT NULL,
  "role" user_role_enum NOT NULL,
  "status" entity_status_enum NOT NULL DEFAULT 'published',
...
CREATE TABLE "attribute" (
  ...
  "is_template_customizable" boolean NOT NULL DEFAULT false,
...
COMMENT ON COLUMN "attribute"."is_overridable" IS 'Cho phép ghi đè giá trị qua Fragment ở tầng Config hay không — false = giá trị luôn lấy từ template_frame/default, Fragment bị bỏ qua khi resolve.';
COMMENT ON COLUMN "attribute"."is_template_customizable" IS 'true = Attribute "chính gốc", giá trị thật sự khác nhau giữa các Template, Template cần tự khai báo template_frame riêng. false = Attribute "đơn giản", Template tự động lấy default_value từ tầng Attribute, không cần khai báo lại.';
...
CREATE TABLE "product_intent_element" (
  ...
  "cdc_version" bigint NOT NULL DEFAULT 1,
  "cdc_timestamp" timestamp NOT NULL DEFAULT (now()),
  PRIMARY KEY ("product_intent_id", "ot_core_code", "element_type_code", "leg")
);
...
COMMENT ON TABLE "product_intent_element" IS '#16 Product Intent — Element nền khai báo cho Intent, đủ cấu trúc OT lõi/leg (Giai đoạn 66, mirror obligation_type_composition) để phân biệt OE thuộc OT lõi Cốt lõi hay Phụ trợ.';
...
ALTER TABLE "product_intent_element" ADD FOREIGN KEY ("ot_core_code") REFERENCES "obligation_type_core" ("code") DEFERRABLE INITIALLY IMMEDIATE;

ALTER TABLE "product_intent_element" ADD FOREIGN KEY ("element_type_code") REFERENCES "obligation_element_type" ("code") DEFERRABLE INITIALLY IMMEDIATE;
